...
public:
	...
	static void nullStack() {
		std::cerr << std::endl << "Гномик магокрад заметил фокусы с пустым стеком! Он следит!" << std::endl;
		system("Pause");
	}
	static void outOfRandge() {
		std::cerr << std::endl << "Гномик магокрад заметил фокусы с пустотой! Он следит!" << std::endl;
		system("pause");
	}
};
...
public:
	...
	class StackIterator {
		using StackType = std::stack<std::vector<xto_im>>;
		StackType* stackPtr; StackType tempStack;
	public:
		StackIterator(StackType* stackPtr) : stackPtr(stackPtr), tempStack(*stackPtr) {}

		bool hasNext() const {
			return !tempStack.empty();
		}

		std::vector<xto_im> next() {
			if (hasNext()) {
				auto vec = tempStack.top();
				tempStack.pop();
				return vec;
			}
			else
			{
			magicSpellsRejector::outOfRandge();
			throw;
			}
		}
	};
	...
	class VectorIterator {
		using VectorType = std::vector<xto_im>;
		VectorType* vectorPtr; size_t index;
	public:
		VectorIterator(VectorType* vectorPtr) : vectorPtr(vectorPtr), index(0) {}

		bool hasNext() const {
			return index < vectorPtr->size();
		}
	...
	StackIterator getStackIterator() {
		return StackIterator(thisIsVectorISwear.get());
	}
	
	VectorIterator getVectorIterator(std::vector<xto_im>& vec) {
		return VectorIterator(&vec);
	}

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// лучше бы файловые потоки сказали добавить >:T
